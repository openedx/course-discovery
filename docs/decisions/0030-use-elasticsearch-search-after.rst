...
A new mixin, `SearchAfterMixin`, will be created to enable the search_after functionality in catalog query endpoints, such as `/api/v2/catalog/query_contains`.
This mixin serves as a replacement for `PkSearchableMixin` in various models. It also introduces custom parameters, such as queryset and documents, to enable filtering at the query level, minimizing the need to fetch all entries from Elasticsearch.
