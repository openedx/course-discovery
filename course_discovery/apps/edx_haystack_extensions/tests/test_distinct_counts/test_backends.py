...
from course_discovery.apps.edx_haystack_extensions.distinct_counts.backends import (
    DistinctCountsSearchQuery, DistinctCountsElasticsearchBackendWrapper
)
...
class DistinctCountsSearchQueryTests(ElasticsearchTestMixin, TestCase):
    def test_clone(self):
        ...
        query._aggregation_key = 'aggregation_key'
        query._distinct_hit_count = 123

        clone = query._clone()
        assert query.facets == clone.facets
        assert query._aggregation_key == clone._aggregation_key
    ...
    def test_clone_with_different_class(self):
        ...
        assert query.facets == clone.facets
        assert not hasattr(clone, '_aggregation_key')
        assert not hasattr(clone, '_distinct_hit_count')

    def test_set_aggregation_key(self):
        """ Verify that set_aggregation_key sets the aggregation_key."""
        query = DistinctCountsSearchQuery()
        assert query._aggregation_key is None
        query.set_aggregation_key('aggregation_key')
        assert query._aggregation_key == 'aggregation_key'
    ...
    def test_get_distinct_count_runs_query_when_cache_empty(self):
        ...
        run_1 = CourseRunFactory(title='foo', course=course)
        run_2 = CourseRunFactory(title='foo', course=course)
    ...
    def test_run_executes_the_query_and_caches_the_results(self):
        ...
        run_4 = CourseRunFactory(title='bar', pacing_type='instructor_paced', hidden=False, course=course_2)
