...
from course_discovery.apps.course_metadata.models import (
    Course, CourseEditor, CourseEntitlement, CourseRun, CourseUrlSlugHistory, Organization, Program, Seat, SeatType,
    Video
)
...
class CourseViewSet(CompressedCacheResponseMixin, viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if url_slug:
            ...
            if CourseUrlSlugHistory.objects.filter(url_slug=url_slug, partner=partner).exists():
    ...
    def update_course(self, data, partial=False):
        ...
        if url_slug:
            validators.validate_slug(url_slug)
            all_course_historical_slugs = CourseUrlSlugHistory.objects.filter(url_slug=url_slug, partner=course.partner)
