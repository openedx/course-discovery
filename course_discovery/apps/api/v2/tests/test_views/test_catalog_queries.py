...
class CatalogQueryViewSetTests(ElasticsearchTestMixin, APITestCase):
    ...
    def test_incorrect_queries(self):
        ...
        failed_query_error_message = {
            "detail": "Failed to make Elasticsearch request. Got exception: RequestError(400, "
                      "'search_phase_execution_exception', 'Failed to parse query [(title:)]')"
        }
        qs = urllib.parse.urlencode({
            'query': 'title:',
            'course_run_ids': self.course_run.key,
        ...
        response = self.client.get(url)
        assert response.status_code == 400
        assert response.data == failed_query_error_message
