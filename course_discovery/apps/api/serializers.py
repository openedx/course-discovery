...
class MinimalCourseRunSerializer(DynamicFieldsMixin, TimestampModelSerializer):
    ...
    class Meta:
        ...
        fields = ('key', 'uuid', 'title', 'image', 'short_description', 'marketing_url', 'seats', 'start', 'end',
                  'publish_date', 'enrollment_start', 'enrollment_end', 'pacing_type', 'type', 'status',)
    ...
    def validate(self, data):
        ...
        publish_date = data.get('publish_date', self.instance.publish_date if self.instance else None)

        if start and end and start > end:
            raise serializers.ValidationError({'start': _('Start date cannot be after the End date')})

        if publish_date and start and publish_date > start:
            raise serializers.ValidationError({'publish_date': _('Go Live date cannot be after the Start date.')})
        ...
        return [
            {
                ...
                'publish_date': course_run.publish_date,
