...
class DataLoaderMixin:
    ...
    def call_course_api(self, method, url, data):
        ...
        Expects `self.api_client` to be defined in the subclass.
        """
    ...
    def create_course(self, data, course_type, course_run_type_uuid):
        """
        ...
        request_data = self.create_course_api_request_data(data, course_type, course_run_type_uuid)
        response = self.call_course_api('POST', url, request_data)
        if response.status_code not in (200, 201):
            logger.info(f"Course creation response: {response.content}")
        return response.json()

    def create_course_api_request_data(self, data, course_type, course_run_type_uuid):
        """
        Given a data dictionary, return a reduced data representation in dict
    ...
    def update_course_api_request_data(self, data, course, is_draft):
        pass
