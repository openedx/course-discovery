...
class EcommerceApiDataLoaderTests(ApiClientTestMixin, DataLoaderTestMixin, TestCase):
    ...
    def compose_warning_log(self, alt_course, alt_currency, alt_mode, product_class):
        ...
        if alt_course:
            ...
            msg += 'mode ' + alt_mode
        msg += ' while loading {product_class}'.format(product_class=product_class)
    ...
    @responses.activate
    @ddt.data(
        ({'key': 'entitlement', 'value': 'entitlement'}),
        ({'key': 'enrollment_code', 'value': 'enrollment code'})
    )
    def test_invalid_stockrecord(self, product_class):
        self.mock_courses_api()
        products = self.mock_products_api(valid_stockrecord=False, product_class=product_class['key'])
        with mock.patch(LOGGER_PATH) as mock_logger:
            self.loader.ingest()
    ...
    @responses.activate
    @ddt.data(
        ('a01354b1-c0de-4a6b-c5de-ab5c6d869e76', None, None, {'key': 'entitlement', 'value': 'entitlement'}),
        (None, "NRC", None, {'key': 'enrollment_code', 'value': 'enrollment code'}),
        (None, None, "notamode", {'key': 'entitlement', 'value': 'entitlement'})
    )
    @ddt.unpack
    def test_ingest_fails(self, alt_course, alt_currency, alt_mode, product_class):
        ...
        self.mock_products_api(
            ...
            alt_mode=alt_mode,
            product_class=product_class['key']
        )
        with mock.patch(LOGGER_PATH) as mock_logger:
            self.loader.ingest()
            msg = self.compose_warning_log(alt_course, alt_currency, alt_mode, product_class['value'])
            mock_logger.warning.assert_any_call(msg)
