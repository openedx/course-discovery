...
from course_discovery.apps.course_metadata.data_loaders.constants import (
    ...
    CourseEditorIngestionErrors,
    CourseEditorIngestionErrorMessages,
)
...
class CourseEditorsLoader(AbstractDataLoader, DataLoaderMixin):
    ...
    def __init__(
        ...
        self.error_logs = {key: [] for key in LOADER_ERROR_LOG_SEQUENCE}
            ...
            if action == 'add':
                if not set(Organization.user_organizations(user)).intersection(course.authoring_organizations.all()):
                    self.log_ingestion_error(
                        ...
                        CourseEditorIngestionErrorMessages.USER_DOES_NOT_BELONG_TO_ORG.format(
                    ...
                    continue
                try:
                    CourseEditor.objects.get_or_create(user=user, course=course)
                    self.ingestion_summary['success_count'] += 1
                except Exception as e:
                    self.log_ingestion_error(
                        CourseEditorIngestionErrors.COURSE_EDITOR_CREATE_ERROR,
                        CourseEditorIngestionErrorMessages.COURSE_EDITOR_CREATE_ERROR.format(
                            user_identifier=user_identifier,
                            course_title=course.title,
                            index=index,
                            exception=str(e)
                ...
                self.log_ingestion_error(
                    CourseEditorIngestionErrors.MISSING_REQUIRED_DATA,
                    f"[Row {index}] Unsupported action '{action}' for course editor."
                )

        logger.info("Course editor ingestion complete.")
        logger.info(f"Ingestion Summary: {self.ingestion_summary}")
        self.render_error_logs(self.error_logs, LOADER_ERROR_LOG_SEQUENCE)
    ...
    def get_user(identifier):
        ...
        """
        return User.objects.filter(username=identifier).first() or User.objects.filter(email=identifier).first()
    ...
    def get_course(identifier):
        ...
        try:
            ...
            return Course.everything.filter(uuid=uuid_obj).first()
        except (ValueError, TypeError):
            return Course.everything.filter(key=identifier).first()
