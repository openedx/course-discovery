...
class Course(DraftModelMixin, PkSearchableMixin, CachedMixin, TimeStampedModel):
    ...
    def set_active_url_slug(self, slug):
        ...
        logger.info('The current slug is {}'.format(self.url_slug))
        logger.info('The slug to be set to is {}.'.format(slug))
        logger.info('Current course is a draft: {}'.format(self.draft))
