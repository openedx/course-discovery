    ...
    def test_create_comment_for_course_case_salesforce_case_id_set(self):
        ...
        with mock.patch(salesforce_path) as mock_salesforce:
            with mock.patch(create_case_path) as mock_create_case_for_course:
                ...
                mock_salesforce().CaseComment.create.assert_called_with({
                    'ParentId': course.salesforce_case_id,
                    'CommentBody': util._format_user_comment_body(user, body, None)  # pylint: disable=protected-access
                })
        ...
        return_value = {
            'records': [
                ...
                },
                {
                    ...
                    'CommentBody': 'This is an internal user comment without formatting.'
                },
            ]
        }
        ...
        with mock.patch(salesforce_path):
            with mock.patch(query_path, return_value=return_value) as mock_query:
                ...
                mock_query.assert_called_with(
                    "SELECT CreatedDate,CommentBody,CreatedBy.Email FROM CaseComment "
                    "WHERE ParentId='{}' AND IsDeleted=FALSE ORDER BY CreatedDate ASC".format(course.salesforce_case_id)
                )
                self.assertEqual(comments, [
                    {
                        'email': 'test@example.com',
                        'course_run': 'course-v1:testX+TestX+Test',
    ...
    def test_format_and_parse(self):
        ...
        with mock.patch(salesforce_path):
            ...
            expected_formatted_message = '[User]\n{}\n\n[Course Run]\n{}\n\n[Body]\n{}'.format(
                '{} {} ({})'.format(user.first_name, user.last_name, user.email), course_run_key, body
            )
            self.assertEqual(
                formatted_message,
            ...
            parsed_message = util._parse_user_comment_body(  # pylint: disable=protected-access
                {
                    'CommentBody': formatted_message
                }
            )
            self.assertEqual(parsed_message.get('email'), user.email)
            self.assertEqual(parsed_message.get('course_run'), course_run_key)
