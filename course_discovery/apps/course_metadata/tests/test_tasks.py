...
from course_discovery.apps.course_metadata.models import BulkOperationTask, CourseType, Organization, ProgramType, Course
...
from course_discovery.apps.course_metadata.tasks import (
    process_bulk_operation, update_org_program_and_courses_ent_sub_inclusion, process_send_course_deadline_email
)
    ...
    def test_process_send_course_deadline_email_success(self, mock_send_email):
        ...
        with LogCapture(LOGGER_PATH) as log_capture:
            ...
            log_capture.check(
                (
                    LOGGER_PATH, 'INFO',
                    f"Sending course deadline email for course '{self.course.title} ({self.course.key})' to recipients: {self.recipients}"
                ),
            )
