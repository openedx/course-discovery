...
class BackpopulateCourseTypeCommandTests(TestCase):
    ...
    def test_invalid_args(self):
        ...
        assert cm.value.args[0] == 'No courses found. Did you specify an argument?'

        with pytest.raises(CommandError) as cm:
            self.call_command(course_arg)  # no partner
        assert cm.value.args[0] == 'You must specify --partner'
        ...
        assert cm.value.args[0] == 'Supplied Course Type slug [NotACourseType] does not exist.'

        with pytest.raises(CommandError) as cm:
            self.call_command('--allow-for=verified-audit:NotACourseRunType', partner_code, course_arg)
        assert cm.value.args[0] == 'Supplied Course Run Type slug [NotACourseRunType] does not exist.'
